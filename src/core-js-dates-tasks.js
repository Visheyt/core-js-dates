...
function formatDate(date) {
  ...
  return Intl.DateTimeFormat('en-US', options).format(fullData);
}
...
function getNextFridayThe13th(date) {
  ...
  let isFind = false;
  while (!isFind) {
    const friday = new Date(year, month, 13);
    if (friday.getDay() === 5) {
      isFind = true;
      return friday;
    }
...
function getWorkSchedule(period /* countWorkDays, countOffDays */) {
  return new Date(period.end);
  // const schedule = [];
  // const dateRange = new Date(start).getDate() - new Date(end).getDate();
...
function isLeapYear(date) {
  if (
    Number(date.getFullYear()) % 4 === 0 &&
    Number(date.getFullYear()) % 100 !== 0
  ) {
    return true;
  }
  if (
    Number(date.getFullYear()) % 400 === 0 &&
    Number(date.getFullYear()) % 4 === 0 &&
    Number(date.getFullYear()) % 100 === 0
  ) {
    return true;
